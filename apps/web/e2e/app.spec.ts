...
async function waitForSample(page: Page) {
  await page.goto("/")
  await expect(page.getByLabel("Document title")).toHaveValue("Event-driven architecture")
...
test("edits geometry, undoes, saves, and restores a document", async ({ page }) => {
  ...
  await title.fill("Checkout architecture")
  ...
  await page.reload()
  await expect(title).toHaveValue("Checkout architecture")
...
test("captures the redesigned desktop and narrow layouts", async ({ page }) => {
  await mkdir(screenshotDirectory, { recursive: true })
  await waitForSample(page)
  await page.screenshot({ path: `${screenshotDirectory}/redesign-desktop.png`, fullPage: true })
  await page.setViewportSize({ width: 390, height: 844 })
  await expect(canvas(page)).toBeVisible()
  await page.screenshot({ path: `${screenshotDirectory}/redesign-mobile.png`, fullPage: true })
})
...
test("captures routed arrows across offset, reversed, close, and crossing layouts", async ({ page }) => {
  ...
  await expect(page.getByLabel("Document title")).toHaveValue("Arrow routing review")
...
async function openSelectionFixture(page: Page) {
  ...
  await expect(page.getByLabel('Document title')).toHaveValue('Subsystem editing')
}
...
test('floating document sidebar keeps separate documents, edits, imports and active state', async ({ page }) => {
  ...
  const title = page.getByLabel('Document title')
  await title.fill('My first system')
  await title.press('Enter')
  await toggle.click()
  ...
  await title.fill('Another system')
  await title.press('Enter')
  await list.getByRole('button', { name: 'My first system', exact: true }).click()
  await expect(title).toHaveValue('My first system')
  ...
  await expect(title).toHaveValue('Another system')
  expect((await saveDocument(page)).document.nodes).toHaveLength(0)
  await page.reload()
  await expect(title).toHaveValue('Another system')
  ...
  await expect(title).toHaveValue('Imported system')
...
test('migrates the previous single-document autosave without losing it', async ({ page }) => {
  ...
  await page.goto('/')
  await expect(page.getByLabel('Document title')).toHaveValue('Existing user work')
  await expect(page.getByRole('navigation', { name: 'Saved documents' }).getByRole('button', { name: 'Existing user work' })).toBeVisible()
  expect((await saveDocument(page)).document.nodes[0]).toMatchObject({ id: 'original', label: 'Keep me', x: 2, y: 3 })
  await page.getByRole('button', { name: 'New document', exact: true }).click()
  await page.getByRole('navigation', { name: 'Saved documents' }).getByRole('button', { name: 'Existing user work' }).click()
  await expect(page.getByLabel('Document title')).toHaveValue('Existing user work')
